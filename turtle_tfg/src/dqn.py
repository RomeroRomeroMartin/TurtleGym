...
orientacion=0
old_state=actualiza_estado((0,0),goal,orientacion)
print('ESTADO: ',old_state)
done=False
n_acciones=0
pos_actual=[0,0]
while not done:
    action=tcp_dqn(old_state)
    action=int(action)
    ...
    print(n_acciones)
    print(pos_actual,orientacion,new_pos,new_orientacion)
    old_state=new_state
    pos_actual=new_pos
    ...
    #raw_input('Pulsa ENTER para siguiente acción')
